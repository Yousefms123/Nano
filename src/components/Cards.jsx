...
const Card = ({ item, index, openIndex, handleShareClick, handleCopyLink }) => {
		...
		<div className="h-full bg-white rounded-xl transition-shadow duration-300 border border-gray-200 font-Tajawal">
			<div className="relative h-64 w-full overflow-hidden rounded-t-xl">
				...
				<div className="absolute top-4 left-4">
					...
					>
						{openIndex === index ? (
							<X size={20} className="text-main-color" />
						) : (
			...
			<div className="p-5 flex flex-col justify-between h-64">
				<div className="mb-3 flex items-center justify-between">
					...
					<div className="flex items-center gap-1 px-2 py-1 rounded-full">
						<p className="text-base text-gray-600">{item.rate}</p>
						<Star size={18} className="fill-amber-500 stroke-amber-500" />
					</div>
				</div>
				...
				{/* الزر في الأسفل */}
				<Link
					to={`/donate/item?title=${encodeURIComponent(item.title)}`}
					className="w-full"
				>
					<button
...
// Reusable CardCarousel Component
const CardCarousel = ({ title, data, renderItem }) => {
	return (
